...
>
	{#snippet row(item: RequestSummary)}
		<td class="px-3 py-2 whitespace-nowrap">
			<a class="font-medium hover:underline" style="color: var(--color-brand);" href={`${base}/requests/${item.id}`}>
		...
		<td class="px-3 py-2 max-w-[22rem]">
			<span class="block truncate font-mono text-xs" title={item.original_uri}>{truncateMiddle(item.original_uri, 60)}</span>
			<span class="text-fg-muted text-xs">{item.request_kind}</span>
		</td>
