...
`GET /api/storage/summary` returns an authenticated PostgreSQL storage footprint overview for llmtrace-owned tables. It uses a fixed allowlist of internal relation names and reports relation presence, estimated row counts, total/table/index/auxiliary bytes, and last vacuum/analyze timestamps for request traces, archive metadata/blob tables, rollups, sessions, messages, audit events, UI sessions, and OAuth states. The endpoint reads PostgreSQL catalog/statistics views only; it does not scan or return request URLs, headers, bodies, API key hashes, session identities, audit details, or plugin metadata.

`POST /api/redaction/preview` accepts optional sample `headers`, `uri`, and `body` fields and returns the current configured redaction result without proxying, storing, or writing audit/trace rows. It is intended for validating header and URI redaction settings before sending real traffic. Header samples are capped at 64 entries with 128-byte names and 8 KiB values; URI samples are capped at 8 KiB; body samples are capped at 64 KiB and are returned unchanged because persisted request/response payloads are used for internal audit.
