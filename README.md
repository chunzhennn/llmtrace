...
`GET /api/ui-sessions?limit=100&offset=0` lists active UI login sessions for operational review; set `include_expired=true` to include rows waiting for retention cleanup. Items include a SHA-256 `session_hash`, user/display name, login method, creation and expiry timestamps, remaining TTL, and expired status, but never return the raw session cookie token. Use `DELETE /api/ui-sessions/{session_hash}` to revoke one UI login session by its hash. Successful revocations insert a `ui_session_revoked` audit event with the revoked hash.
...
Request list responses include `page.has_more` and `page.next_offset` so clients can request the next page without assuming all matching requests were returned. Invalid timestamps, a `since` value later than `until`, negative duration bounds, `min_duration_ms > max_duration_ms`, an unknown `request_kind`, an unknown `status_class`, or a malformed `session_id` return `400 Bad Request`. Session list search matches `session_key`, `user_id`, and `user_name`; use `limit` and `offset` to page through long session lists:

`DELETE /api/requests/{id}` removes one captured request trace for incident cleanup or accidental sensitive capture. The delete runs in a transaction, cascades stored session messages for that request, decrements the matching minute rollup counters without going below zero, removes the trace session if that session no longer has requests, and writes a `request_trace_deleted` audit event. The response returns only low-sensitivity metadata about the deleted trace and whether rollup/session cleanup occurred; it does not return headers, bodies, URLs, API key hashes, or plugin metadata.

`GET /api/requests/export.jsonl` accepts the same query parameters as `GET /api/requests` and downloads the current filtered page as newline-delimited request-summary JSON. Responses use `application/x-ndjson`, include `Content-Disposition: attachment`, and expose the exported row count in `x-llmtrace-export-rows`.
...
Session list responses include `page.has_more` and `page.next_offset` so clients can request the next page without assuming all matching sessions were returned. `GET /api/sessions/{id}` returns session metadata, `request_stats`, and a bounded page of messages. `request_stats` includes request/error counts, bytes in/out, captured body bytes, average and maximum duration/TTFT, and first/last request timestamps. `GET /api/sessions/{id}/requests?limit=100&offset=0` returns the same request summary shape as `GET /api/requests`, scoped to that session, with `page.has_more` and `page.next_offset`. `GET /api/sessions/{id}/requests/export.jsonl?limit=500&offset=0` exports the same scoped request page as newline-delimited JSON and includes `x-llmtrace-export-rows`. `GET /api/sessions/{id}/messages/export.jsonl?messages_limit=500&messages_offset=0` exports the current bounded message transcript page as newline-delimited JSON with the same row-count header. Use `messages_limit` and `messages_offset` to page through long sessions:
...
`GET /api/config` returns an authenticated runtime configuration summary for admin UIs and operational checks. It reports deployment mode, public/listen addresses, proxy capture and WebSocket limits, upstream allowlist entries, storage pool and retention settings, auth/OAuth/rate-limit posture, observability token presence, redaction mode, and plugin count. It intentionally omits secret-bearing fields such as `storage.postgres_url`, local admin usernames/passwords/password hashes, OAuth client IDs/secrets and allowlist values, metrics bearer tokens, and plugin filesystem paths.

`GET /api/security/posture` returns an authenticated production-readiness checklist over the loaded runtime config. It reports pass/warn/fail checks for deployment mode, public URL HTTPS posture, secure cookies, body redaction, retention, upstream allowlisting, metrics authentication, local password handling, login rate limiting, and OAuth allowlisting. The endpoint returns static check IDs and messages only; it does not echo secret values. Non-HTTPS `server.public_url` is reported as a warning rather than a hard failure because some deployments intentionally terminate HTTPS at a reverse proxy while the service itself sees HTTP.

`GET /api/retention/status` returns an authenticated read-only retention overview. It reports whether retention is enabled, the configured retention days, current cutoff, prune interval and batch size, and aggregate counts of rows that are currently eligible for pruning across request traces, minute rollups, empty trace sessions, UI audit events, expired UI sessions, and expired OAuth states. The endpoint estimates prunable rows only; it does not delete data or return trace, audit detail, header, body, API key, or plugin metadata fields.

`GET /api/storage/summary` returns an authenticated PostgreSQL storage footprint overview for llmtrace-owned tables. It uses a fixed allowlist of internal relation names and reports relation presence, estimated row counts, total/table/index/auxiliary bytes, and last vacuum/analyze timestamps for request traces, rollups, sessions, messages, audit events, UI sessions, and OAuth states. The endpoint reads PostgreSQL catalog/statistics views only; it does not scan or return request URLs, headers, bodies, API key hashes, session identities, audit details, or plugin metadata.

`POST /api/redaction/preview` accepts optional sample `headers`, `uri`, and `body` fields and returns the current configured redaction result without proxying, storing, or writing audit/trace rows. It is intended for validating production redaction settings before sending real traffic. Header samples are capped at 64 entries with 128-byte names and 8 KiB values; URI samples are capped at 8 KiB; body samples are capped at 64 KiB. When `redaction.body_redaction = "disabled"`, the body preview is unchanged, so use synthetic examples when testing secret-bearing payloads.

`GET /api/data/overview?since_hours=24` returns an authenticated data coverage and freshness overview for admin dashboards. Request totals come from minute rollups, while the recent window reports aggregate request/error counts, distinct upstream/model/session counts, bytes, latency averages/maxima, first/last timestamps, session activity, audit event counts, and active/expired auth state counts. `since_hours` defaults to 24 and is capped at 2160 hours. The endpoint does not return request URLs, headers, bodies, API key hashes, session identities, audit details, or plugin metadata.

`GET /api/data/integrity?since_hours=24` returns an authenticated read-only storage consistency check for operations dashboards. It compares minute rollups with raw `request_traces` aggregates over the bounded lookback window, reports raw-vs-rollup deltas for request/error/byte/latency counters, returns up to 50 mismatched buckets, and reports relationship anomalies such as requests that reference missing sessions or stored messages that reference missing parents. `since_hours` defaults to 24 and is capped at 2160 hours. The endpoint returns aggregate counts and bucket timestamps only; it does not return request URLs, headers, bodies, API key hashes, session identities, message content, audit details, or plugin metadata.
...
`GET /api/usage/api-keys?since_hours=24&limit=10` returns Top-N usage by stored `api_key_hash`, excluding requests without a key hash. Each item includes request and error counts, distinct session count, bytes in/out, captured body bytes, average and maximum duration/TTFT, and first/last seen timestamps. Use the returned `api_key_hash` with `GET /api/requests?api_key_hash=...` to drill into that key's request list without exposing raw API keys.

`GET /api/usage/models?since_hours=24&limit=10` returns Top-N model usage buckets. Each item includes request and error counts, error rate, proxy-error and HTTP-5xx counts, distinct upstream/API-key/session counts, bytes in/out, captured body bytes, average/maximum/p95 duration, average/maximum/p95 TTFT, and first/last seen timestamps. It is intended for model adoption and reliability dashboards without exposing per-key or per-request detail.

`GET /api/usage/users?since_hours=24&limit=10` returns Top-N usage grouped by session identity (`trace_sessions.user_id` and `user_name`), with requests that lack session identity grouped as `unknown`. Each item includes request and error counts, error rate, proxy-error and HTTP-5xx counts, distinct session/API-key/upstream counts, bytes in/out, captured body bytes, average/maximum/p95 duration, average/maximum/p95 TTFT, and first/last seen timestamps. It returns aggregate counts only and does not include API key hash values, headers, bodies, URLs, or plugin metadata.

`GET /api/usage/upstreams?since_hours=24&limit=10` returns Top-N upstream health buckets by `upstream_host`. Each item includes request and error counts, error rate, proxy-error count, HTTP 2xx/3xx/4xx/5xx/no-status counts, distinct session count, bytes in/out, captured body bytes, average/maximum/p95 duration, average/maximum/p95 TTFT, and first/last seen timestamps. It intentionally stays at host-level aggregation and does not include URLs, headers, bodies, API key hashes, or plugin metadata.

`GET /api/usage/errors?since_hours=24&limit=10` returns a dashboard-oriented error overview over the same bounded lookback style. It reports total failed requests, proxy-error and HTTP-5xx counts, affected session count, average and maximum duration, first/last failed request timestamps, and Top-N breakdowns by error source, upstream host, model, status class, and request kind. It intentionally returns aggregate buckets rather than raw error strings, headers, or bodies.

`GET /api/usage/latency?since_hours=24&limit=10` returns latency percentiles for dashboard tail-latency panels. It includes totals and Top-N breakdowns by upstream host, model, and request kind, with request counts, duration sample counts, average/max duration, p50/p90/p95/p99 duration, TTFT sample counts, average/max TTFT, and p50/p90/p95/p99 TTFT. `since_hours` defaults to 24 and is capped at 2160 hours; `limit` defaults to 10 and is capped at 50.
